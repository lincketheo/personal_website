---
title: How to Build a 32 bit x86 Bootloader
date: 2023-02-03
categories: [Computer Science, Operating Systems]
tags: [os, bootloader, software]
---
# Introduction
Today, we’ll build a bootloader from scratch. It’s generally suggested that you should have some assembly experience before embarking on your bootloader journey, but personally, I learned assembly by building a bootloader. So don’t fret if you don’t know any. I’ll be using intel assembly compiled using nasm in this post. I won't be going over assembly syntax much in this tutorial, though, because I think what's most helpful for learning assembly is having someone handhold you through writing programs until you feel comfortable writing your own code.

## Notes
Throughout this tutorial, I'll try to point you in the right direction for the two intel manuals I've been learning from ([^fn1] and [^fn2]) using the following syntax (section a.b.c [^fn1]). These are by no means comprehensive sections, but a good place to start looking for something related to what I just said. As always, the source of truth for all of this information is not some internet blog written by an amateur software engineer, it's the intel manuals. However, it is pretty understandable to not want to read each one of their ~500 page manuals fully. Just start getting comfortable using them as a last resort if anything on the internet isn't fully spelled out (I promise my tutorial will have flaws in it).
...
1.  It loads whatever code runs _after_ the bootloader (usually the operating system kernel) into memory. We’ll do this by loading the first sector of our booting disk into memory (because our kernel is so small in this tutorial) using the _real mode_ interrupt [0x13, ah=0x02](https://en.wikipedia.org/wiki/INT_13H#INT_13h_AH=02h:_Read_Sectors_From_Drive).

2. General system initialization (initializing the GDT (section 3.4.5[^fn2]), stack, segmentation (section 3.3[^fn2]) etc.) that you don't want to do in the kernel. In this tutorial, we'll initialize the GDT by defining a global **code** and **data** section. We'll also set up the stack and assume the flat memory model without paging (section 3.2.2[^fn2]). It's my goal to implement paging soon, and I'll update this tutorial once I've set up paging.

4.  Switching from [16 bit real mode](https://en.wikipedia.org/wiki/Real_mode) to [32 bit protected mode](https://en.wikipedia.org/wiki/Protected_mode) (section 9.9.1[^fn2]). This is a pretty powerful step because it disables all bios interrupts and lets us use all 32 bits and segmentation set up from our GDT. Without protected mode, we can only access 16 bits of address space, or `64 KiB` (which is pretty bad). 
...
A bootloader is stored on some sort of storage, usually our hardrive, but this could also be a flash drive, floppy disk, or whatever medium we wish to boot from. BIOS recognizes bootloaders by the magic bytes `0xaa55` in bytes 510 and 511. Let's write an assembly program that, when compiled, has `0xaa55` in bytes 510 and 511.

First, let's just define a word `0xaa55`:
```
dw 0xaa55
```

Somehow we need to place this word on byte 510. We can do that by writing 510 bytes, then executing our command:
```
times 510 db 0
dw 0xaa55
```
...
jmp $
```
(ie jump to my current memory address). All together:
```
jmp $
times 510 db 0
dw 0xaa55
...
Now wait a second, we've just written an instruction, followed by writing 510 bytes. In Von Neumann Architecture, data and code are technically the same thing. So our instructions take up space in our program. The above code looks like this in memory `feeb000000...aa55` (where there are 1020 0's after `feeb`. That makes 514 bytes! (510 bytes of 0's + 2 bytes of `feeb` + 2 bytes of `aa55` = 514) See for yourself: compile this program:
```bash
$ nasm boot.asm
$ stat boot
  File: boot
...
One last thing. In order to do anything meaningful, we need to reference address starting at 0x7c00 because that's the location that BIOS loads our program into memory. To do this, you can add the following to your bootloader:
...
Remember that only 512 bytes of our bootloader are loaded. Let's write a simple "kernel" after our program that prints the character 'Q' in real mode, and jump to 0x7e00 (0x7c00 + 512 bytes) - ie, the code after our bootloader:
```
jmp 0x7e00
times 510 - ($-$$) db 0
dw 0xaa55
...
If we compile and run this, nothing happens. That's because all the code after `dw 0xaa55` is on disk, and not in memory. To fix this, we need to load our disk into memory using int 0x13. We want to read cylinder (ch) 0, head (dh) 0 and sector (cl) 2. _Sector starts at 1_ and our bootloader was on sector 1, so our "kernel" is on sector 2:
...
To simplify things, we can just load 0 into es (the base of our segment) and 0x7e00 into bx:
```
mov ax, 0
mov es, ax
mov bx, 0x7e00
...
We'll set `ah` to 2 to indicate a read of disk:
```
mov ah, 0x02
```

And indicate that we want to read 1 drive (al):
```
mov al, 1
```
...
Try running it again and you'll see, as expected, our "kernel code" print a Q to the screen, then infinitely loop.

## Set up the GDT
In this section, we'll define our gdt in a seperate file called `gdt.asm`. Let's label our gdt and two sections:
...
The next bits (16-31) are the first (of three) part of the base address, which we will call 0:
```
dw 0x0000
```
...
Bits 15-12 (decreasing) of byte offset 4 are the tricky ones. They consist of (s, dpl and p):

- p = 1: indicates that this is  a valid segment (if 0, an exception will be thrown)
- dpl = 00: permission level 0 (most priveleged)
- s = 1: Code segment
...
Using the flat memory model, we'll let both the code and data segment take up the same memory space. The data segment has a similar derivation as the code descriptor, with some minor differences.
...
We'll actually load the gdt in the next section
...
lgdt [gdtr]
```
4. Actually enter protected mode by or-ing cr0 with 0x01:
```
mov eax, cr0
or eax, 0x1
mov cr0, eax
...
In protected mode, we can just call int 0x10 to print a character anymore. We need to explicitly set the vga to our character. Putting everything together:
